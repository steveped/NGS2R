#' @title A wrapper for the bash shell command fastqc.
#'
...
#' 
#' The component \code{$complete} is the complete message as output by \code{fastqc}, in order or completion
...
#' @export
fastqc <- function(files, outDir, threads=floor(detectCores()/2), args=c(), exec="fastqc"){
  
  # Check the given set of files
  test <- sapply(files, FUN=file.exists)
  if (length(which(!test)) >= 1) stop("\nCouldn't find files: \n", paste(files[!test], collapse="\n"))
  test <- grep("(.fastq|fastq.gz|.fq|.fq.gz|.bam|.sam)$", basename(files), invert=TRUE)
  if (length(test) >=1 ) stop("\nFiles can only be fastq, sam or bam files: \n", paste(files[test], collapse="\n"))
  ...
  if (missing(outDir)) stop("\nThe variable outDir must be provided")
  if (!file.exists(outDir)) dir.create(outDir, recursive=TRUE)
  
  # Set the arguments
  o <- paste("-o", outDir)
  t <- paste("-t", max(1, threads)) # at least one core...
  
  v <- system(paste(exec, "-v"), intern=TRUE)
